...
def one_market_rows(book: pd.DataFrame, ps: PriceSeries) -> pd.DataFrame:
    ...
    for slug, g in book.groupby("slug", sort=False):
        up = g[g.outcome.str.startswith("up")]
        dn = g[g.outcome.str.startswith("down")]
        if up.empty or dn.empty:
            continue
        u, d = up.iloc[-1], dn.iloc[-1]
        win_start = int(u.win_start); end_ts = int(u.end_ts)
        ...
        if not (open_px > 0 and spot > 0 and close_px > 0 and math.isfinite(rv60) and 0.02 < rv60 < 5.0):
            continue
        ...
        rows.append({
            "slug": slug, "cond": str(u.cond), "win_start": win_start, "end_ts": end_ts,
            "decision_ts_ms": decision_ts, "side": side, "fair": fair, "p_up": p_up,
...
def main():
    ...
    manifest = {
        ...
        "rules":{
            "settlement_only":True,"decision_s2c":DECISION_S2C,"ticket":TICKET,
            ...
            "fair":"digital N(d2) using Chainlink spot/threshold and causal Chainlink 60m realized volatility",
        ...
        "markets_with_decision_rows":int(len(raw)),
    }
