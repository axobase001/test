...
EXECUTION_GRADE = "STRICT_TAPE_COARSE_BOOK_10S"
EPS = 1e-9
...
def fee_factor(price: float, rate: float, exponent: float) -> float:
    if not (0 < price < 1 and rate >= 0 and exponent >= 0):
        return math.nan
...
class ClobStaticCache:
    ...
    def get(self, condition_id: str) -> MarketStatic | None:
        if condition_id not in self.data:
            ...
            if obj is None:
                ...
                self.data[condition_id] = {
                    "status": "ok",
                    "tokens": tokens,
                    "fee": {"r": fd.get("r"), "e": fd.get("e"), "to": fd.get("to")},
        ...
        if row.get("status") != "ok":
            return None
        try:
            rate = float(row["fee"]["r"]); exponent = float(row["fee"]["e"])
            taker_only = bool(row["fee"]["to"])
        except Exception:
            return None
        ...
        if not toks or not any(v.lower() == "up" for v in toks.values()) or not any(v.lower() == "down" for v in toks.values()):
            return None
        return MarketStatic(condition_id, toks, rate, exponent, taker_only)
...
def cluster_bootstrap_roi(fills: pd.DataFrame, n: int = 10000, seed: int = 20260815) -> list[float] | None:
    ...
    for i in range(n):
        j = rng.integers(0, len(agg), size=len(agg))
        vals[i] = pnl[j].sum() / stake[j].sum()
...
def grade_execution(
    ...
    later = token_rows[
        (token_rows["ts_ms"] >= decision_ts + latency_ms)
        & (token_rows["ts_ms"] <= decision_ts + tape_window_ms)
    ].sort_values("ts_ms")
    if later.empty:
        return {"filled": False, "reason": "no_persist_snapshot", "decision_depth_shares": decision_depth}
    p = later.iloc[0]
    persist_ask = float(p.best_ask)
    ...
    if persist_ask > target_px + EPS or persist_depth <= 0:
        return {
            "filled": False, "reason": "book_not_persistent",
            "decision_depth_shares": decision_depth,
            "persist_ts_ms": int(p.ts_ms), "persist_ask": persist_ask,
            "persist_depth_shares": persist_depth,
        }

    tape = trade_rows[
        (trade_rows["ts_ms"] >= decision_ts + latency_ms)
    ...
    if tape.empty:
        return {
            "filled": False, "reason": "no_buy_tape_corroboration",
            "decision_depth_shares": decision_depth,
            "persist_ts_ms": int(p.ts_ms), "persist_ask": persist_ask,
            "persist_depth_shares": persist_depth,
        }
    ...
    capacity_shares = max(min(decision_depth, persist_depth, tape_size), 0.0)
    capacity_usd = capacity_shares * target_px
    return {
        "filled": capacity_shares > 0,
        "reason": "strict_witness" if capacity_shares > 0 else "zero_capacity",
        "decision_depth_shares": decision_depth,
        "persist_ts_ms": int(p.ts_ms), "persist_ask": persist_ask,
        "persist_depth_shares": persist_depth,
        "tape_ts_ms": int(witness.ts_ms), "tape_price": float(witness.price),
        "tape_size_shares": tape_size, "tape_tx_hash": str(witness.tx_hash),
        "capacity_shares": capacity_shares, "capacity_usd": capacity_usd,
    }
...
def process_day(
    ...
    book = pd.read_parquet(book_path, columns=bcols)
    trades = pd.read_parquet(trade_path, columns=tcols)
    ...
    book["ts_ms"] = to_ms(book["timestamp"]); trades["ts_ms"] = to_ms(trades["timestamp"])
    ...
    for market_id, g in book.groupby("market_id", sort=True):
        ...
        if not meta:
            continue
        open_ts_s = int(meta["open_ts_s"]); close_ts_ms = int(meta["close_ts_ms"])
        condition_id = str(meta["condition_id"])
        outcome = resolutions.get(market_id)
        if outcome not in ("Up", "Down"):
            continue
        open_bn = bn.open_price(open_ts_s)
        if not (open_bn > 0 and math.isfinite(open_bn)):
            continue
        g = g.sort_values(["ts_ms", "token_id"])
        chosen = None
        for ts_ms, snap in g.groupby("ts_ms", sort=True):
            ...
            if not (math.isfinite(p_rv) and math.isfinite(p_iv)):
                continue
            # Mapping-agnostic pre-gate: ignoring fees can only admit extra candidates, never drop a real 3c signal.
            fairs = [min(p_rv, p_iv), 1.0 - max(p_rv, p_iv)]
            if max(f - a for f in fairs for a in asks) < threshold:
                continue
            static = clob.get(condition_id)
            if static is None or not static.taker_only:
                continue
            row_by_outcome = {}
            ...
            candidates = []
            for side in ("Up", "Down"):
                r = row_by_outcome[side]
                ask = float(r.best_ask)
                ff = fee_factor(ask, static.fee_rate, static.fee_exponent)
                fair = conservative_fair(side, p_rv, p_iv)
                edge_equiv = fair - ask - ask * ff
                edge_exact = fair * (1.0 - ff) - ask
                candidates.append((edge_equiv, edge_exact, side, r, ask, ff, fair))
            eligible = [x for x in candidates if x[0] >= threshold]
            if not eligible:
                continue
            edge_equiv, edge_exact, side, r, ask, ff, fair = max(eligible, key=lambda x: (x[0], x[1]))
            chosen = {
                "ts_ms": int(ts_ms), "close_ts_ms": close_ts_ms, "cid": market_id,
                "condition_id": condition_id, "token_id": str(r.token_id), "action": f"buy_{side.lower()}",
                "outcome_token": side, "resolved_outcome": outcome, "target_px": ask,
                "fee_rate": static.fee_rate, "fee_exponent": static.fee_exponent,
                "fee_factor": ff, "fee_equiv_per_gross_share": ask * ff,
        ...
        grade = grade_execution(
            token_rows, tg, chosen["ts_ms"], chosen["target_px"], chosen["decision_ask_levels"],
            latency_ms, tape_window_ms,
        )
        chosen.update(grade)
        chosen.pop("decision_ask_levels", None)
        signals.append(dict(chosen))
        if not grade.get("filled") or float(grade.get("capacity_usd", 0.0)) + EPS < base_stake:
            continue
        net_ratio = 1.0 - float(chosen["fee_factor"])
        if net_ratio <= 0:
            continue
        effective_cost = float(chosen["target_px"]) / net_ratio
...
def main() -> None:
    ...
    markets_path = hf_file("markets/all.parquet", hf_cache)
    resolutions_path = hf_file("resolutions/all.parquet", hf_cache)
    markets = pd.read_parquet(markets_path)
    resolutions_df = pd.read_parquet(resolutions_path)
    ...
    lo = pd.Timestamp(start, tz="UTC"); hi = pd.Timestamp(end + timedelta(days=1), tz="UTC")
    m = m[(m["start_dt"] >= lo) & (m["start_dt"] < hi)].copy()
    market_meta = {
        str(r.market_id): {
    ...
    resolutions = settlement_map(resolutions_df)
    print(f"Eligible BTC15m markets in metadata: {len(market_meta)}", flush=True)
    ...
    clob = ClobStaticCache(args.cache / "clob_static.json")
    signals: list[dict] = []; fills: list[dict] = []
    for d in daterange(start, end):
        print(f"DAY {d.isoformat()} start", flush=True)
        s, f = process_day(
            d, market_meta, resolutions, bn, der, clob, args.threshold,
            args.latency_ms, args.tape_window_ms, args.base_stake, hf_cache,
        )
    ...
    fee_schedules = sorted({
        (float(x.get("fee_rate")), float(x.get("fee_exponent"))) for x in signals
        if x.get("fee_rate") is not None and x.get("fee_exponent") is not None
    })
    ...
    summary = {
        ...
        "policy": {
            ...
            "signal_edge": "conservative fair minus ask minus historical taker fee-equivalent at decision ask",
            "diagnostic_exact_net_ev": "conservative fair*(1-fee_factor)-ask",
        ...
        "historical_fee": {
            "fee_schedules_seen": fee_schedules,
            "formula": "fee_factor = r * (p*(1-p))**e; BUY fee is deducted in outcome shares; effective cost per net share = p/(1-fee_factor)",
            "market_static_source": "official CLOB /clob-markets/{condition_id}",
        },
        "markets_metadata": len(market_meta),
        "mapping_api_calls": clob.calls,
        ...
        "caveat": "Order book observations are approximately 10-second snapshots. This is stricter than snapshot-only replay because real trade tape is required, but it is intentionally labeled coarse-book rather than millisecond-exact execution.",
    }
